...
package object compat extends compat.PackageShared {
  final implicit class MutableTreeMapExtensions2(private val fact: m.TreeMap.type) extends AnyVal {
    def from[K: Ordering, V](source: TraversableOnce[(K, V)]): m.TreeMap[K, V] =
      build(m.TreeMap.newBuilder[K, V], source)
  }

  final implicit class MutableSortedMapExtensions(private val fact: m.SortedMap.type)
      extends AnyVal {
