...
class BreakoutRewrite(addCompatImport: RuleCtx => Patch)(implicit val index: SemanticdbIndex) {
  ...
  val toSpecificCollectionWithImport = Map(
    "scala.collection.immutable.SortedMap" -> "toImmutableSortedMap",
    "scala.collection.immutable.HashMap"   -> "toImmutableHashMap",
    "scala.collection.immutable.ListMap"   -> "toImmutableListMap",
    "scala.collection.immutable.TreeMap"   -> "toImmutableTreeMap",
    "scala.collection.mutable.SortedMap"   -> "toMutableSortedMap",
    "scala.collection.mutable.HashMap"     -> "toMutableHashMap",
    "scala.collection.mutable.ListMap"     -> "toMutableListMap",
    "scala.collection.mutable.TreeMap"     -> "toMutableTreeMap",
    "scala.collection.mutable.Map"         -> "toMutableMap",
    "scala.collection.immutable.IntMap"    -> "toImmutableIntMap",
    "scala.collection.immutable.LongMap"   -> "toImmutableLongMap",
    "scala.collection.mutable.LongMap"     -> "toMutableLongMap"
  )

  val toSpecificCollection = toSpecificCollectionBuiltIn ++ toSpecificCollectionWithImport
  ...
  def apply(ctx: RuleCtx): Patch = {
    ...
    def covertToCollection(intermediateLhs: String,
      ...
      val patchSpecificCollection =
        toSpecificCollection
          .get(toCollection)
          .map(toX => ctx.addRight(ap0, '.' + toX))
          .getOrElse(Patch.empty)

      if (toSpecificCollectionWithImport.contains(toCollection)) {
